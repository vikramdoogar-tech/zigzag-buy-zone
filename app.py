...
st.set_page_config(page_title="ZigZag Terminal", page_icon="📈", layout="wide", initial_sidebar_state="expanded")
...
.gn{color:#22c55e;}.ye{color:#eab308;}.rd{color:#ef4444;}.bl{color:#60a5fa;}
.mt{font-size:0.72rem;color:#94a3b8;margin-bottom:5px;}
.pr{display:flex;flex-wrap:wrap;gap:8px;margin-top:6px;}
.pc{min-width:72px;}.pl{font-size:0.62rem;color:#94a3b8;}.pv{font-size:0.85rem;font-weight:600;color:#e2e8f0;}
...
def get_json_file(filename: str):
    try:
        r = requests.get(GITHUB_BASE + filename, timeout=30)
        r.raise_for_status()
        return r.json(), None
...
def get_prices(symbols):
    ...
    out = {}
    for sym in symbols:
        tickers = PRICE_ALIASES.get(sym, [f"{sym}.NS", f"{sym}.BO"])
        for tkr in tickers:
            try:
                info = yf.Ticker(tkr).fast_info
                px = None
                if info:
                    px = info.get("lastPrice") or info.get("last_price") or info.get("regularMarketPrice")
                if px:
                ...
                pass
            try:
                hist = yf.Ticker(tkr).history(period="7d", auto_adjust=False)
                if hist is not None and not hist.empty:
                    out[sym] = round(float(hist["Close"].dropna().iloc[-1]), 2)
                    break
            except Exception:
                pass
        time.sleep(0.05)
...
def recalc_level_pack(price, crash_buy, avg_drop, rot_target):
    l1 = round(crash_buy, 2)
    l2 = round(crash_buy * (1 - avg_drop * 0.15 / 100), 2)
    l3 = round(crash_buy * (1 - avg_drop * 0.30 / 100), 2)
    sl = round(l3 * 0.95, 2)
    up = round((rot_target / price - 1) * 100, 1) if price > 0 and rot_target > 0 else 0.0
    dist = round((price / crash_buy - 1) * 100, 2) if crash_buy > 0 else 0.0
    return l1, l2, l3, sl, up, dist

def make_card(sym, tier, sec, ab, dist, spd, score, rr, win, roty, plbl, pval, l1, l2, l3, sl, tgt, up, rd, dd, el, dlf, css, dc):
    sg = "+" if dist > 0 else ""
    ac = "bg" if ab else "br"
...
stock_raw, stock_err = get_json_file("data_stocks_slim.json")
rebound_raw, rebound_err = get_json_file("data_rebound.json")
watch_raw, watch_err = get_json_file("data_watchlist.json")

STOCK_DATA = normalize_stock_rows(stock_raw)
REBOUND_DATA = normalize_rebound_rows(rebound_raw)
MY_WATCHLIST = normalize_watchlist(watch_raw)
...
with tab1:
    pool = [s for s in STOCK_DATA if s["score"] >= msco and s["rr"] >= mrr and s["tier"] in tiers and (not ab_only or s["above_200"]) and s["crash_buy"] > 0]
    ...
    for s in pool:
        ...
        if p is None or cb <= 0 or rt <= 0:
            continue
        l1, l2, l3, sl, up, dist = recalc_level_pack(p, cb, s["avg_drop"], rt)
        rows.append({**s, "live": p, "dist": dist, "l1": l1, "l2": l2, "l3": l3, "sl": sl, "up": up})
    ...
    def show_buy_card(r, css, dc):
        ...
        if sp > 0 and r["live"] < r["rot_target"]:
            ...
            days_left = "-"
        st.markdown(make_card(r["sym"], r["tier"], r["sector"], r["above_200"], r["dist"], sp, r["score"], r["rr"], r["win_pct"], r["rot_yr"], "LIVE", r["live"], r["l1"], r["l2"], r["l3"], r["sl"], r["rot_target"], r["up"], rd, dd, elapsed, days_left, css, dc), unsafe_allow_html=True)
...
with tab2:
    ...
    rebound_prices = get_prices([r["sym"] for r in REBOUND_DATA])
    rb2 = []
    for r in REBOUND_DATA:
        ...
        if tags == "Broader NSE" and not (r["above"] and r["price"] >= 20):
            continue
        live_price = rebound_prices.get(r["sym"], r["price"])
        r2 = dict(r)
        r2["price"] = live_price
        if r2["l1"] > 0:
            r2["dist"] = round((live_price / r2["l1"] - 1) * 100, 2)
        r2["up_scan"] = round((r2["target"] / live_price - 1) * 100, 1) if live_price > 0 else 0
        rb2.append(r2)

    iz2 = [r for r in rb2 if -3 <= r["dist"] <= 3]
    nr2 = [r for r in rb2 if 3 < r["dist"] <= pr2]
    bw2 = [r for r in rb2 if r["dist"] < -3]
    sk = {"Fastest rebound": lambda x: -x["speed"], "Closest to zone": lambda x: x["dist"], "Best upside %": lambda x: -x["up_scan"], "Highest score": lambda x: -x["score"], "Most rotations/yr": lambda x: -x["roty"]}[srt2]
    iz2.sort(key=sk); nr2.sort(key=sk); bw2.sort(key=sk)

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("In Zone", len(iz2)); c2.metric("Approaching", len(nr2)); c3.metric("Below Zone", len(bw2)); c4.metric("Filtered", len(rb2))
    st.divider()

    def show_rebound_card(r, css, dc):
        sp = r.get("speed", 0)
        rd = int(r.get("rdays", 0)); dd = int(r.get("ddays", 0))
        ...
        st.markdown(make_card(r["sym"], tg, "", r["above"], r["dist"], sp, r["score"], r["rr"], r["win"], r["roty"], "LIVE", r["price"], r["l1"], r["l2"], r["l3"], r["sl"], r["target"], r["up_scan"], rd, dd, "-", dlf, css, dc), unsafe_allow_html=True)

    if shz and iz2:
        st.subheader(f"In Zone ({len(iz2)})")
        for r in iz2[:40]: show_rebound_card(r, "cg", "gn")
    if sha and nr2:
        st.subheader(f"Approaching ({len(nr2)})")
        for r in nr2[:40]: show_rebound_card(r, "cy", "ye")
    if shb and bw2:
        st.subheader(f"Below Zone ({len(bw2)})")
        for r in bw2[:40]: show_rebound_card(r, "cb", "bl")
...
with tab3:
    ...
    inv = 0.0; cur = 0.0; wins = 0; losses = 0
    for w in MY_WATCHLIST:
        lp = wll.get(w["sym"]); qty = w.get("qty", 0); entry = w.get("entry", 0)
        if lp is not None and qty > 0:
            inv += qty * entry; cur += qty * lp
            if lp >= entry: wins += 1
            else: losses += 1

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Tracked", len(MY_WATCHLIST)); c2.metric("Above entry", wins); c3.metric("Below entry", losses)
    if inv > 0:
        pnl = cur - inv
    ...
    if s3 == "% vs entry": dl.sort(key=lambda x: x["chg"] if x["chg"] is not None else -9999, reverse=True)
    elif s3 == "Symbol": dl.sort(key=lambda x: x["sym"])
    else: dl.sort(key=lambda x: x["up"] if x["up"] is not None else -9999, reverse=True)
    ...
    for w in dl:
        ...
        if lp >= tgt and tgt > 0: css, ic = "ct", "T"
        elif lp >= entry: css, ic = "cg", "+"
        elif sl > 0 and lp <= sl: css, ic = "cr", "X"
        else: css, ic = "cy", "~"
        gc = "gn" if chg >= 0 else "rd"; vs = f"{qty} shares | ₹{qty*lp:,.0f}" if qty > 0 else "Watching"
        st.markdown(
            ...
            f'</div><div class="tb">{thesis}</div></div>', unsafe_allow_html=True)
